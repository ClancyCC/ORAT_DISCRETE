...
from agent_CNN_LSTM_discrete import CQLSAC_CNN_LSTM_Discrete, CQLSAC_CNN_Discrete
...
def get_config():
    ...
    parser.add_argument("--run_name", type=str, default="CQL-SAC-active-tracking_agent-discrete", help="Run name, default: CQL-SAC")
...
def load_Buffer(buffer ,path ,config):
    ...
    # for d in range(0,1):
        ...
        # actions
        act_tmp = np.array([np.array(x) for x in dict_tmp['action']])[:-1]
        # rewards
        ...
        for i in range(0, state_tmp.shape[0]):
            ...
            buffer.add(
                ...
                torch.from_numpy(np.array(act_tmp[i])).float().cuda(),
    ...
    with logger:
        if 'deva' in config.input_type.lower() or 'image' in config.input_type.lower() or 'Mask' in config.input_type.lower():
            if 'cnn' in config.input_type.lower() and 'lstm' in config.input_type.lower():
                agent = CQLSAC_CNN_LSTM_Discrete(state_size=(3, 64, 64),
                                        action_size=9,
                                        tau=config.tau,
                ...
                agent = CQLSAC_CNN_Discrete(state_size=(3, 64, 64),
                                        action_size=9,
                                        tau=config.tau,
                                        hidden_size=config.hidden_size,
                                        learning_rate=config.learning_rate,
                ...
                #                         action_size=9,
                #                         ...)
                # TODO: implement discrete MLP agent if needed
                raise NotImplementedError("Discrete MLP agent not implemented yet.")


        if 'devadepth' in config.input_type.lower() or 'rgbd' in config.input_type.lower():
            if 'cnn' in config.input_type.lower() and 'lstm' in config.input_type.lower():
                agent = CQLSAC_CNN_LSTM_Discrete(state_size=(4, 64, 64),
                                        action_size=9,
                                        tau=config.tau,
                                        hidden_size=config.hidden_size,
                                        learning_rate=config.learning_rate,
                ...
                agent = CQLSAC_CNN_Discrete(state_size=(4, 64, 64),
                                   action_size=9,
                                   tau=config.tau,
                                   hidden_size=config.hidden_size,
                                   learning_rate=config.learning_rate,
